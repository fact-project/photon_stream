...
# Storage
We offer two storage formats. First the [Json](https://www.json.org/)-Lines format, and second a dedicated binary format. Json-Lines is excellent to share data with collegues or new students. Json is human readable and fortunately surprisingly efficient to store the photon-stream. Many programming languages can read and write Json.
...
The photon-stream ```pass4``` was created based on best effort. However, we are aware of a few shortcomings which might be adressed in future passes. Here is a list of possible improvements for the future in no particular order.
