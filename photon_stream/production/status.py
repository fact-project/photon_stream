...
def status(photon_stream_dir, known_runs_database='known_runs.msg'):
    ...
    for index, row in tqdm(info.iterrows()):
        ...
        if info['fRunTypeKey'][index] == observation_key:
            if info['photon_stream_NumTrigger'][index]==0 and not info['photon_stream_exists'][index]:
                ...
                if os.path.exists(run_path):    
                    info['photon_stream_exists'][index] = True
                    info['photon_stream_NumTrigger'][index] = tools.number_of_events_in_run(run_path)
