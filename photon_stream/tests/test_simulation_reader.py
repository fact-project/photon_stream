...
import tempfile
import os
from photon_stream.simulation_truth import corsika_headers as ch
...
def test_read_and_write_MMCS_CORSIKA_headers():
    ...
    with tempfile.TemporaryDirectory(prefix='phs_test_corsika') as tmp:
        with open(os.path.join(tmp, 'headers_out'), 'wb') as fout:
            ch.append_corsika_headers_to_file(
                headers=headers_in,
                fout=fout
            )

        with open(os.path.join(tmp, 'headers_out'), 'rb') as fin:
            headers_back = ch.read_corsika_headers_from_file(fin)

    np.testing.assert_equal(
        headers_back['run_header'], headers_in['run_header']
    )
    np.testing.assert_equal(
        headers_back['event_headers'], headers_in['event_headers']
    )
    np.testing.assert_equal(
